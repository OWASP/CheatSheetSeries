...
- For Issue (Comments of the issue do not contain the info when
             a PR is referenced):
    - Has assignees
...
# Define constants
# API to retrieve the list of Issues/PR (GitHub REST API v3 considers every
# pull request an issue, but not every issue is a pull request)
# Ask to the API to sort the list by the updated date in order to have the
# oldest on the top on the list
# See https://developer.github.com/v3/issues/#list-issues-for-a-repository for
# explanation
ISSUE_API = "https://api.github.com/repos/OWASP/CheatSheetSeries/issues?"\
            "page=1&per_page=1000&sort=updated&direction=asc"
# Expiration delay
MAX_MONTHS_ALLOWED = 1

# Define utility function: Cf criteria in the comment of the script for
# the criterias
...
def is_old_issue(issue):
    ...
    return has_assignees and (not has_help_wanted_label and not
                              has_internal_label)
...
if response.status_code != 200:
    print("Cannot load the list of Issues/PR content: HTTP %s received!" %
...
if (len(old_issues["PR"]) + len(old_issues["ISSUE"])) != 0:
    ...
    if len(old_issues["PR"]) > 0:
        buffer += "Old pull request identified (%s items): %s\n" % (len(
                   old_issues["PR"]), " / ".join(old_issues["PR"]))
    if len(old_issues["ISSUE"]) > 0:
        buffer += "Old issue identified (%s items): %s\n" % (len(
            old_issues["ISSUE"]), " / ".join(old_issues["ISSUE"]))
else:
    buffer += "State: Nothing identified!"
print(buffer)

# Send notification the project management channel on Slack if the url of the
# webhook is passed as unique first parameter
if len(sys.argv) == 2:
    if (len(old_issues["PR"]) + len(old_issues["ISSUE"])) == 0:
        color = "good"
    else:
        color = "warning"
    message = '{\"text\": \"Old PR and Issue identification watchdog\",\
                "attachments\": [ {\"fallback\": \"%s\",\"color\":\"%s\",\
                "title\": \"Status\",\"text\": \"%s\"}]}' % (
                    color, buffer, buffer)
    ...
    response = requests.post(sys.argv[1],
                             ...
                             data=message)
    if response.status_code != 200:
        print("Cannot send notification to slack: HTTP %s received!" %
              response.status_code)
        sys.exit(2)
