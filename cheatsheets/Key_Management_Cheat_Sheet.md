...
- **Rotation Schedule**: Establish and document a key rotation schedule based on the system's threat model and risk assessment ([OWASP ASVS 5.0 §13.1.4](https://github.com/OWASP/ASVS/blob/master/5.0/en/0x22-V13-Configuration.md#v131-configuration-documentation), [§13.3.4](https://github.com/OWASP/ASVS/blob/master/5.0/en/0x22-V13-Configuration.md#v133-secret-management)).
- **Representative Cryptoperiod Examples**:
    ...
    - **TLS/HTTPS Server Keys and Certificates**: Lifetimes for publicly trusted certificates are capped by the [CA/Browser Forum Baseline Requirements](https://cabforum.org/working-groups/server/baseline-requirements/documents/) rather than by NIST cryptoperiod guidance, and the maximum validity period is being shortened over successive ballots. Follow the current Baseline Requirements, and generate a new key pair at each renewal instead of reusing the existing one.
...
- **Usage-Based Rotation Limits**: Key rotation must account for usage volume in addition to calendar time. For example, when using AES-GCM with random 96-bit IVs, a single key MUST NOT be used for more than `2^32` (4,294,967,296) encryption operations, to prevent IV collisions and tag forgery ([NIST SP 800-38D §8.3](https://csrc.nist.gov/pubs/sp/800/38/d/final)).
- **Handling Existing Encrypted Data (Envelope Encryption)**: When rotating keys with envelope encryption, re-wrap stored Data Encryption Keys (DEKs) under the new Key Encryption Key (KEK) without re-encrypting the underlying bulk data payload. For managing multiple active keys, key IDs, and migration trade-offs for direct DEK rotation, see the [Cryptographic Storage Cheat Sheet](Cryptographic_Storage_Cheat_Sheet.md#key-lifetimes-and-rotation).
...
- **Crypto-Erase and Software Memory**: For software-managed keys, prefer cryptographic erasure, which destroys the key protecting the stored ciphertext and so renders the data unrecoverable without overwriting it ([NIST SP 800-57 Part 1 Rev. 5](https://csrc.nist.gov/pubs/sp/800/57/pt1/r5/final) Section 6.2.2.1, [NIST SP 800-88 Rev. 1](https://csrc.nist.gov/pubs/sp/800/88/r1/final)). Overwriting key material in memory is best-effort in managed runtimes (such as Java, .NET, Python, Go, or JavaScript), where copies can survive in garbage-collected memory, immutable strings, or swapped-out pages. Where the platform provides secure-memory or zeroization APIs, use them.
