...
Inserting the CSRF token in the HTTP request header via JavaScript, is considered more secure than adding in the hidden field form parameter. In this situation, even if the CSRF token is weak, predictable or leaked but still an attacker cannot forge the POST request directly by setting the custom request header through XmlHttpRequest. This type of implementation gives an extra layer of defense. 
