...
With a CSRF vulnerability on login form, an attacker can make a victim login with an account they control. The attacker can then learn behaviour from their interaction with the site with features such as search/activity/navigation history. The victim might also end up trusting the site to perform operations such as providing highly confidential information such as payment details assuming that they are logged into their account. For more information about login CSRF and it's associated risks, see section 3 of [this](https://seclab.stanford.edu/websec/csrf/csrf.pdf) paper.
