...
If one-way storage is specifically required (for example because the session database is accessible to other applications or to backup operators), split the cookie into a public identifier used to look up the session row and a secret verifier whose hash is what the server stores. On each request:

- Parse `(identifier, verifier)` from the cookie.
- Look up the session row by `identifier`.
- Constant-time compare `SHA-256(encoded_verifier)` against the stored hash.
