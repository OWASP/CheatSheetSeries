...
- Credential-guessing attacks commonly leverage password wordlists, common-password dictionaries, and credentials obtained from previous data breaches. These behaviors are reflected in [MITRE ATT&CK T1110 – Brute Force](https://attack.mitre.org/techniques/T1110/), including T1110.001 (Password Guessing), T1110.003 (Password Spraying), and T1110.004 (Credential Stuffing). Because these techniques rely on predictable passwords or previously compromised credentials,passphrases composed of multiple uncommon words can make dictionary-based guessing more difficult by reducing the effectiveness of commonly used attacker wordlists. NIST notes that estimating entropy for user-chosen passwords is challenging, so entropy estimates should not be interpreted as precise guarantees of password strength ([NIST SP 800-63B – Strength of Passwords](https://pages.nist.gov/800-63-4/sp800-63b/passwords/#appA)).
