...
- **Problems with multi-domain sites**: The current implementation does not allow the webmaster to provide a allow list of domains that are allowed to frame the page. While whitelisting can be dangerous, in some cases a webmaster might have no choice but to use more than one hostname.
