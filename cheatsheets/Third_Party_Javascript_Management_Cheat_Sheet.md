...
I define the use case attack surface as the combination of three values:
UCAS1:the number of bytes of code executed for a given use case, user action or user story; (more code provides more risk of insecure code).
UCAS2:the number of source code files involved in executing that use case; (more source files implies more developers and developer teams with different levels of secure code knowledge, especially with microservice architectures).
UCAS3: the number of different vendors that have code executing for that use case. (the more open source, marketing and analytic vendor code and proprietary 3rd party libraries that are used, the greater the insecure code risk).
