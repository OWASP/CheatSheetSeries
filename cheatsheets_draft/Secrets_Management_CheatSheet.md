...
- Key handling and cryptographic implementations are taken care of by Encryption Secvice, not by developers
...
1. Revokation: Keys that were exposed should ensure immediate revokation. They secret must be able to be de-authorized quickly and systems must be in place to identify the revokation status of a secret.
1. Rotation: A new secret must be able to be quickly created and implemented, preferrably via an automated process to ensure repeatability, low rate of implementation error, and least-privilege (not directly human-readable).
