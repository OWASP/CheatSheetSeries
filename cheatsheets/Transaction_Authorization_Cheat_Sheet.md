...
- A card that has a transaction authorization number (TAN),
...
Some of these can be implemented with a physical device or in a mobile application.
...
Since developers cannot assume that a user's computer is secure, the data for a typical transaction should be presented and acknowledged using an external authorization component.
...
Deciding which transaction data are significant should be based on:
...
Developers must prevent users from authorizing fraudulent operations. Problems occur if an application requires a user to perform the same actions for authentication and transaction authorization. Consider the following example:
...
Some applications only ask for transaction authorization credentials once (such as a static password, code sent through SMS, or a token response). Afterwards, the user can authorize any transaction during the entire session or reuse the same credentials each time they need to authorize a transaction. If developers use this approach, attackers can employ malware to sniff credentials and use them to authorize any transaction without the user's knowledge.

## 2. Non-functional guidelines

### 2.1 Authorization should be performed and enforced server-side

Like [all other security controls](https://cwe.mitre.org/data/definitions/602.html) transaction authorization should be enforced on the server side. It should **never** be possible to influence the authorization result by altering the data which flows from a client to a server by (for example):
...
IF multiple transaction authorization methods must be made available to the user, the server must ensure that the transaction occurs with their chosen authorization method or the authorization method enforced by application policies. Otherwise, malware can downgrade an authorization method to a less or even the least secure authorization method. Developers must make it impossible to change a chosen authorization method by manipulating the parameters provided from the client.

This is especially important if developers are asked to add a new authorization method that enhances security. Unfortunately, lazy developers often decide to build a new authorization method on top of an old codebase. If this insecure case occurs, an attacker could manipulate a client to successfully authorize a transaction by sending parameters using the old method, despite the fact that the application has already switched to a new method.

### 2.3 Transaction verification data should be generated server-side

If develoeprs decide to transmit significant transaction data programmatically to an authorization component, they should take extra care into denying any client modifications to the transaction data at authorization. **All significant transaction data must be verified by the user, generated and stored on a server, then passed to an authorization component without any possibility of tampering by the client.**

A flawed and insecure case occurs when developers collect significant transaction data client-side and pass it to the server, which allows malware to manipulate that data and show faked transaction data in an authorization component.

### 2.4 Application should prevent authorization credentials brute-forcing

**Developers must make sure that their application can't allow attackers to use brute-forcing when transaction authorization credentials are sent to the server for verification. After a set number of failed authorization attempts, the entire transaction authorization process should be restarted.** Also, there are other methods to prevent brute-forcing and stop other automation techniques, see [OWASP Authentication Cheat Sheet](Authentication_Cheat_Sheet.md#prevent-brute-force-attacks).
...
**The developers must ensure that the business logic flow for transaction authorization occurs in in sequential step-by-step order so users (or attackers) cannot perform the steps out of order or even skip any of the steps. This should protect against attack techniques such as 1) overwriting transaction data before user will enter the authorization credentials or 2) skipping transaction authorization.**
...
Developers must not allow attackers to modify transaction data after the initial entry by the user. Bad implementations may allow malware to do the following:

- Replay the first step in Section 2.5 (sending transaction data) in the background before the user enters authorization credentials and then overwriting transaction details with a fraudulent transaction.
- Creating and adding new transaction data parameters to a HTTP request which could authorize the transaction. In such a case, a transaction authorization process that is poorly implemented might authorize the initial transaction and then execute a fraudulent transaction (specific example of [Time of Check to Time of Use vulnerability](https://cwe.mitre.org/data/definitions/367.html)).

There are multiple methods that prevent the modification of transaction data during authorization:

- If transaction data is modified, the code could invalidate any previously entered authorization data (e.g. Generated OTP) or the challenge.
- Modifications to transaction data could trigger a vreset of the authorization process.
- Attempts to modify transaction data after the initial user entry is an attack on the system and it should be logged, monitored and carefully investigated.
...
In some malware attacks, a user's authorization credentials are passed to a malware command-and-control server and then used from an attacker-controlled machine. Often, this process is often performed manually by an attacker. To make sure that these are attacks are difficult, the server should only allow transaction authorization to occur in a limited time window between the generation of a challenge (or OTP) and the completion of an authorization. Additionally, such safeguards will also help stop resource exhaustion attacks. This time window should be carefully selected so it will not disrupt normal users' behavior.

### 2.10 Authorization credentials should be unique for every operation

To prevent many replay attacks, each set of authorization credentials should be unique for every operation. They can be achieved using different methods depending on the applied transaction authorization mechanism. For example: developers can use a timestamp, a sequence number, or a random value in signed transaction data or as a part of a challenge.
