...
The CSRF token can be added through hidden fields, headers, and can be used with forms, and AJAX calls. That token should be added to all state changing actions. Make sure that the token is not leaked in the server logs, or [in the URL](#disclosure-of-token-in-url). The server must reject the requested action if the CSRF token fails validation.
