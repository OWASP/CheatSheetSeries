...
- **Allow list allowed extensions. Only allow safe and critical extensions for business functionality**
...
- Double extensions, _e.g._ `.jpg.php`, where it circumvents easily the regular expression `\.jpg`
- Null bytes, _e.g._ `.php%00.jpg`, where `.jpg` gets truncated and `.php` becomes the new extension
- Generic bad regular expression that isn't properly tested and well reviewed. Refrain from building your own logic unless you have enough knowledge on this topic.
...
This can be done preferrably in a allow list approach; otherwise, this can be done in a deny list approach.
...
- Restrict characters to an allowed subset specifically, such as alphanumeric characters, hyphen, spaces, and periods
    - If this is not possible, deny list dangerous characters that could endanger the framework and system that is storing and using the files.
