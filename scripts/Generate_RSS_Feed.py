...
PR_API = "https://api.github.com/repos/OWASP/CheatSheetSeries/pulls?"\
        "page=1&per_page=1000&state=closed"
...
print("[+] Process the obtained list and generate the feed in memory {} items)"
      .format(len(pull_requests)))
...
feed_generator.author({"name": "Core team",
                       "email": "dominique.righetto@owasp.org"})
feed_generator.link({"href": "https://cheatsheetseries.owasp.org",
                     "rel": "self"})
feed_generator.link({"href": "https://github.com/OWASP/CheatSheetSeries",
                     "rel": "alternate"})
feed_generator.language("en")
feed_generator.icon("https://cheatsheetseries.owasp.",
                    "org/gitbook/images/favicon.ico")
...
for pull_request in pull_requests:
    ...
    if pull_request["merged_at"] is None:
        continue
    # Convert merge date from 2019-08-25T06:36:35Z To Sun,
    # 19 May 2002 15:21:36 GMT
    merge_date_src = pull_request["merged_at"]
    merge_date_dst = datetime.strptime(merge_date_src, "%Y-%m-%dT%H:%M:%SZ")
    ...
    for assignee in pull_request["assignees"]:
        contributors.append({"name": assignee["login"],
