...
Any state changing operation requires a secure random token (e.g., CSRF token) to prevent CSRF attacks. A CSRF token should be unique per user session, a large random value, and also generated by a cryptographically secure random number generator. The CSRF token can be added through hidden fields, headers, additional parameters in forms, AJAX calls and within the URL. Token should be added to all state changing actions. Make sure that token will not leakue for example in server logs (e.g. when server log all of the headers) or [in the URL](#disclosure-of-token-in-url). The server rejects the requested action if the CSRF token fails validation.
