...
- **Primary mitigation strategies are based on request type:**
    - For non-form-based applications (e.g., JSON APIs), you can [disallow simple requests](#disallowing-simple-requests) and rely on [custom headers](#employing-custom-request-headers-for-ajaxapi).
    - For form-based applications, the most robust defense is [token-based mitigation](#token-based-mitigation). Always check for [built-in framework support](#use-built-in-or-existing-csrf-implementations-for-csrf-protection) first.
- **Use browser-native features as part of your defense:**
    - Use [Fetch Metadata Headers](#fetch-metadata-headers) to identify and block suspicious requests.
    - Apply [Defense in Depth Techniques](#defense-in-depth-techniques) like the `SameSite` cookie attribute.
- **Address modern attack vectors by understanding and mitigating [Client-Side CSRF](#dealing-with-client-side-csrf-attacks-important).**
- **As a general best practice, do not use GET requests for any state-changing operations.**
...
Login CSRF can be mitigated by creating pre-sessions (sessions before a user is authenticated) and including tokens in login form. You can use any of the techniques mentioned above to generate tokens. Remember that pre-sessions cannot be transitioned to real sessions once the user is authenticated - the session should be destroyed and a new one should be made to avoid [session fixation attacks](http.acrossecurity.com/papers/session_fixation.pdf). This technique is described in [Robust Defenses for Cross-Site Request Forgery section 4.1](https://seclab.stanford.edu/websec/csrf/csrf.pdf). Login CSRF can also be mitigated by including a custom request headers in AJAX request as described [above](#employing-custom-request-headers-for-ajaxapi).
