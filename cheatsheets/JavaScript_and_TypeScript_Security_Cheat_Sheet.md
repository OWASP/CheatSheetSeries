...
Never build code from strings. `eval` and `new Function` execute arbitrary code with the privileges of your page, so any attacker-influenced input reaching them is remote code execution ([CWE-94](https://cwe.mitre.org/data/definitions/94.html)). `setTimeout` and `setInterval` with a string argument compile it the same way through implied `eval`.
...
Nested quantifiers such as `(a+)+` or `(a|ab)*` can make matching take exponential time on crafted input, blocking the single-threaded event loop ([CWE-1333](https://cwe.mitre.org/data/definitions/1333.html)).
...
- Where the overlapping [Node.js Security Cheat Sheet](Nodejs_Security_Cheat_Sheet.md) covers server-side matching timeouts, follow it; on the client, length caps are the primary control.
...
Prototype pollution ([CWE-1321](https://cwe.mitre.org/data/definitions/1321.html)) happens when attacker-controlled keys such as `__proto__`, `constructor`, or `prototype` reach a recursive merge, `Object.assign` target, or path setter, modifying `Object.prototype` for the whole page. No other sheet in this series covers it, which is a structural reason this sheet exists.
...
- Prefer object spread over `Object.assign` for shallow copies of untrusted data: spread defines own properties, while `Object.assign` invokes setters including `__proto__`.
...
Unhandled promise rejections hide failures and crash runtimes. Keep promise chains flat with a terminal error handler, and treat every rejection as a bug to fix rather than noise to suppress. Server-side specifics live in the [Node.js Security Cheat Sheet](Nodejs_Security_Cheat_Sheet.md) and are not repeated here.

For `postMessage`, the receiver must verify `event.origin` against an explicit allowlist and check `event.source` when a conversation partner is expected. The sender must pass an exact `targetOrigin`, never `"*"` for sensitive data. Prefer narrow `MessageChannel` ports over broadcast messaging where the design allows it.
...
- Treat `any` as a hole in every check. Default to `unknown` and narrow it before use.
...
- Lint with `typescript-eslint` recommended sets plus `no-eval`, `no-implied-eval`, and `no-new-func`. Keep dependencies updated so new rules apply.
