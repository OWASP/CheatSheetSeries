...
The attack surface of a web application is generally considered to be the size of the web site, aka the number of page urls, forms, including the number of parameters and parameter values for single page apps, as well as the amount of HTML generated by the server. Various definitions have also included the protocol between the client and the server (TLS). These are basically server side measures. A different measure includes the business logic complexity and the number of use cases or user stories on the site. A use case will involve a sequence of pages or URL parameter values, as well as the metadata of the user to initialize the use case; referrer, user agent, authorization or loyalty member level, language, geolocation etc.

The use case attack surface can be defined as the combination of three values:

1. UCAS1:the number of bytes of code executed for a given use case, user action or user story; (more code provides more risk of insecure code).

2. UCAS2:the number of source code files involved in executing that use case; (more source files implies more developers and developer teams with different levels of secure code knowledge, especially with microservice architectures).

3. UCAS3: the number of different vendors that have code executing for that use case. (the more open source, marketing and analytic vendor code and proprietary 3rd party libraries that are used, the greater the insecure code risk).

The smaller any of those values is, the smaller the use case attack surface is. While some of the executed code, source files and vendors will be the same between different use cases, there are two important points that can be illustrated by the use case attack surface:

1. For many web applications, especially ecommerce sites, the sizes of the use case attack surface on the client side compared to the server side can vary greatly. It is common for the client side to have dozens of vendors supplying megabytes of code in dozens of different javascript files, even for simple use cases which don't involve much code on the server side. In other words, for a given use case, the values for UCAS1, 2 and 3 are often much higher on the client side than the server side, mainly due to the number of 3rd party marketing and analytic javascript code, files and vendors.

2. The code security controls that you have available for the client side use case attack surface and server side use case attack surface of your web application are very different. The server side controls for such risks as the OWASP Top 10 are well known. Some client side controls are discussed below.

This cheatsheet will discuss the risk from third party marketing javascript, also known as 'tags'.
